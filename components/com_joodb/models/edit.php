...
class JoodbModelEdit extends JModelLegacy
{
	...
	public function __construct()
	{
		...
		if ($this->_joobase->published==0) throw new Exception(JText::sprintf( 'Database is unpublished or not availiable'), 404);
        ...
        $item = $this->getData();
		$has_access = JoodbHelper::checkAuthorization($this->_joobase,"accesse",$item);
    ...
    public function getData($id=null)
    {
        ...
        {
            /* Query single object. */
            if (empty($id)) $id = $app->input->getCmd("id");
            if (!empty($id)) {
